  ...
  const getItems = async () => {
    try {
      ...
      if (data && data.length > 0) {
        // Fetching latest chapter optionally (mocked or skipped for now as we don't display it directly on cards yet)
        try {
          const firstChapter = await Mangas.getChapters(data[0].id, 0);
          if (firstChapter && firstChapter.data && firstChapter.data.length > 0) {
            setchaps(firstChapter.data[0].attributes.chapter);
          }
